...
def nse(gt, pred, exposure=1.0):
    gt_spec = gt['spec']
    pred_spec = pred['spec']
    
    gt_norm = np.linalg.norm(gt_spec, ord=1, axis=1, keepdims=True)
    mask = (gt_norm == 0)
    
    diff = np.linalg.norm(gt_spec - pred_spec, ord=1, axis=1, keepdims=True)
    gt_norm[mask] = 1     
    eps = 1e-8
    return diff / (gt_norm + eps)
