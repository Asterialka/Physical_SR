...
def main_processing():
    input_dir = "dataset_h5"
    output_dir = "dataset_rgbs"
    ...
    for filename in tqdm(h5_files, desc="Обработка файлов"):
        ...
        rgb = calculate_rgb(hsi, resampled_sensitivity)
                
        basename = os.path.splitext(filename)[0]
        plt.imshow(rgb / np.max(rgb))
        plt.title(f"RGB: {filename}")
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, f"{basename}.jpg"), bbox_inches='tight', dpi=150)
        plt.close()
